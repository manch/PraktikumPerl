...
#    ToDo:         e, a, d id
#    Added Exception handling, can't access to the db...
##
...
if ($query->param('Execute') && $query->param('cmd')) {
...
}elsif( $query->param('Apply') && $query->param('name') && $query->param('lastname') ){
    my $dbh = DBI::->connect("dbi:SQLite:dbname=myAddManSys.db","","") or die "Could not open Db";
    $id = checkID();
    my $name = $query->param('name');
    my $lastname = $query->param('lastname');
    $dbh->do("INSERT INTO base (id, name, lastname) VALUES ($id,'$name','$lastname')") or die "Could not write to the db";
    $dbh->disconnect;
...
sub newAdress{
	...
	$query->p,
	$query->start_form,
	"Insert your name:",$query->textfield('name'),
	$query->p,
	"Insert your lastname:", $query->textfield('lastname'),
	$query->p,
	$query->submit('Apply'),
	$query->end_form;
...
sub deleteById{
    my ($curId) = @_;
    my $dbh = DBI::->connect("dbi:SQLite:dbname=myAddManSys.db","","") or die "Can't connect to db";
    $dbh->do("DELETE FROM 'base_entry' WHERE id=$curId") or die "Can't delete in the db";
    $dbh->do("DELETE FROM 'base' WHERE id=$curId") or die "Can't delete in the db";
    $dbh->disconnect;
}
